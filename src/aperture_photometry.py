...
def aperture_photometry(
    ...
    for meta, mask in zip(apertures_meta, masks):
        area    = int(mask.sum())
        raw_sum = float(np.nansum(img[mask]))
...
def load_apertures(path: str) -> Tuple[List[Dict], List[np.ndarray], np.ndarray | None]:
    ...
    meta_json = scalar.decode() if isinstance(scalar, (bytes, bytearray)) else str(scalar)
...
def apply_saved_apertures(
    ...
    results = aperture_photometry(img, metas, masks, background=background, annulus_width=annulus_width)
